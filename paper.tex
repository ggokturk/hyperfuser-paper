...
\item \fixme{We propose \acro\footnote{\scriptsize{\url{https://github.com/ggokturk/infuser}}}, an open-source, blazing-fast, error-adaptive, sketch-based Influence Maximization algorithm.} The proposed scheme removes the contamination of sketches in. Error-adaptive sketch rebuilding strategy we employed lets the algorithm rebuild sketches only when necessary, allowing substantial performance gains while retaining high-quality results.  

\item The edges are sampled as they are traversed across several simulations. Thus, sampling, diffusion, and count-distinct processes are fused for all simulations. 
...
% For undirected graphs, a symmetric hash function, i.e $h(u,v) = \mbox{{\sc Hash}}(min(u,v)||max(u,v))~mod~2^{31}$, can be used to produce the same sampling random value for both ends of the edge (u,v)\cite{infuser}.
