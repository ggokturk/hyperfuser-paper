...
%             % \hline
%     {\tt \_mm256\_cmpgt\_epi32} & Compares packed 8x32-bit integers of two input vectors.\\
%     {\tt \_mm256\_max\_epi8} & Compares packed 32x8-bit integers of two input vectors and returns element-wise maximums. \\
%             % \hline
%         % \hline
...
We have tried a few other hash algorithms as well; Ideally, we chose {{\sc Murmur3}} due to its simplicity and good avalanche behavior with maximum bias $0.5\%$\rev{\cite{MurmurHash3Performance}}.
...
% Our implementation generates $B/4$ sample edges and runs $B$ simulations at a time. The dichotomy of two batch sizes comes from SIMD width; 
% sampling probabilities are computed in 31-bit fixed-point precision, 
% whereas since we are using 64-bit hash values, sketch registers only require $\left\lceil log2(64+1) \right\rceil = 6$ bits. Aligned to the nearest data type they can fit, sampling probabilities utilizes 8x32-bit operators, whereas sketch operations employ 32x8bit SIMD vectors. 


% ~works in batches. Each batch sized $B=8$ processed by follows; first, edge hash and threshold are loaded to vector units. The live sample vector, which is stored as 32-bit unsigned integer is initialized with zero.  Then, liveness information is inferred by generating random numbers and comparing them to the edge activation threshold. Random number generation is done by XOR'ing simulation randoms between $X_{r}$ to $X_{r+b}$ and edge hash. Resulting random numbers are considered as fixed-point numbers and compared with the edge threshold $W_{u,v}$ which is also stored as a fixed-point number. After, 8x 32-bit aligned results are converted to compact 8-bit representation by extracting the first bits using the move mask operator. Finally, the result is appended to the live sample vector. Since the live sample vector is a one-hot 32-bit integer, if it is zero, the edge is blocked in all samples and we can skip merging sketch registers.  

% {\bf Regularization.} %FIXme

% {\bf Vectorized Sketch Merge} can work with larger batch size, due to smaller item size. Before the merge, bitwise-AND is performed on live sample vector bits and the inverse of the visited set of source vertex $u$ to find samples where $u$ is not visited. Live sample vector expanded to 32x8bit vector, all 1 bits converted to 255, and 0 bits are converted to 0 bytes in AVX2 vector.

% After expansion, Registers of source and target vertex, $M_u$ and $M_v$, are compared to get element-wise maximums. Then, only live samples were selected from the maximum vector by blending live sample byte vector. Finally, $M_u$ is compared against its previous value to check whether source vertex $u$ will be added to the next iteration's live vertices set.


% \begin{algorithm}
%     \caption{\sc{Vectorized-Merge} ({$u,v,X_r,M_u,M_v,$})}\label{algo:vector-merge}
%     \algorithmicrequire{
%     $u:$ source vertex\\
...
%                     \State continue
%                 \EndIf
%                 \State $live \leftarrow live~\&~\sim visited_{u}[r..32]$
...
%                 % \State $M_u \leftarrow \_mm256\_blendv\_epi8(M_u[r..32],M_v[r..32])$
%                 % % \State $$
...
%                 \State $max \leftarrow 
%                     \_mm256\_max\_epi8(M_u[r..32],~M_v[r..32])$
%                 \State $temp \leftarrow M_u[r..32] $
%                 \State $M_u[r..32] \leftarrow \_mm256\_blendv\_epi8(\newline  
%                 \hspace*{4em}M_u[r..32],~max,~livebytes)$
%                 % \State $$
%                 \State $islive \leftarrow islive~|~\_mm256\_cmp\_ps(M_u[r..32],~temp)$
...
% \begin{figure*}[htbp] 
%     \centering
%     \includegraphics[width=1\linewidth]{images/threads}
%     \centering\caption{Speed-up obtained by \acro{} over {\sc Imm}($\epsilon\myeq 0.13$).
%   \label{fig:scaling} }  
% \end{figure*}
...
For \acro, we used $J=256$ registers which is comparable in terms of quality of other methods. And, for performance we used global estimation error threshold $\epsilon_{global}=0.01$, local estimation error threshold $\epsilon=0.3$, and set non-converged vertices ratio as $\epsilon_{conv}=0.02$. The threshold values are chosen by observations and intuition, simple grid search over threshold most likely would give superior performance but we opt-to leave parameter selection to future work.
...
The performance gap also narrows between \acro and {\sc Skim}, \acro only $30\%$ faster in small $w$. In the same setting, {\sc Tim+}  takes 1.5 hours for {\tt LiveJournal}, \ggx{could not finish {\tt Orkut}; crashing after consuming all memory available}. 
\ggx{On larger} $w=0.1$, \acro is much faster than all other methods and still providing high-quality results. While all methods are struggling with problem size, \acro is converging results much faster, even faster than previous experiment settings. Also, memory consumption stays the same for all experimental settings, much smaller than other methods.
