...
\begin{table}[!ht]
    ...
    \begin{tabular}{|l|p{0.7\linewidth}|}
        ...
        $\Gamma_G(u)$   & Neighborhood of $u$ in graph $G$, the endpoints of its outgoing edges\\ %%FIXME
...
\begin{algorithmic}[1]
    ...
    \For{$k=1\ldots K$}
        \State $s \leftarrow \underset{v\in V}{\mathrm{argmax}}\{${\sc Estimate}$(${\sc Merge}$(M_{S'},M_v))$\}\kktodo{S' nedir}
        \State $S \leftarrow S \cup \{s\}$
        \State $e \leftarrow ${\sc Estimate}$(${\sc Merge}$(M_{S'},M_s))$
        % \State $R_S \leftarrow run\_cascade(G,S,J)$
        \State {Compute seed set $S$'s Reachability set $R_{G}(S)$ \kktodo{buna da bir algoritma eklesek} }
        ...
        \Else
            \For{$ v\in V$} {\bf in parallel}
                ...
                \EndFor
            \EndFor
            \State $M \leftarrow ${\sc Simulate}$(G,M,R_S)$
            \State $M_{S'} \leftarrow zeros(J) $ 
