...
\item We evaluate the performance, memory consumption, and influence score with sketch- and approximation-based state-of-the-art influence maximization algorithms, namely {\sc Skim}~\cite{cohen2014sketch}, {\sc Tim+}~\cite{tim} and {\sc Imm}~\cite{minutoli2019fast}, to accurately position the performance of \acro{} within the IM literature. The experiments show that \acro is $\minspeedup\times$—$\maxspeedup\times$ faster than a state-of-the-art sketch-based algorithm while having almost the same quality with the approximation-based algorithms in terms of influence, and using a comparable amount of memory.
...
\begin{table}[!ht]
    ...
    \begin{tabular}{|l|p{0.7\linewidth}|}
        ...
        $A_G(u)$   & Neighborhood of $u$ in graph $G$, the endpoints of its outgoing edges\\ %%FIXME
        $\Gamma_G(u)$   & Neighborhood of $u$ in graph $G$, the endpoints of its incoming edges\\ %%FIXME
...
% A merge operation for an edge ($u, v$) of the graph is performed on $v$'s register if and only if the edge exists in the sample and \kktodo{bunu da degistirdim}\ggx{biz bunu yapmiyoruz, ben burada kayboldum.} $u$ is {\em live} in the current iteration. 
...
\begin{algorithmic}[1]
    ...
    \For{$ v\in V$} {\bf in parallel}
        ...
        \EndFor
    \EndFor
    \State $M \leftarrow ${\sc Simulate}$(G,M,\emptyset)$
    \State $M_{S'} \leftarrow zeros(J)$
    \State $\varsigma \leftarrow 0$
    \For{$k=1\ldots K$}
        \State $s \leftarrow \underset{v\in V}{\mathrm{argmax}}\{${\sc Estimate}$(${\sc Merge}$(M_{S'},M_v))$\}\kktodo{S' nedir}\ggx{ latent seed set after rebuild}
        \State $S \leftarrow S \cup \{s\}$
        \State $e \leftarrow ${\sc Estimate}$(${\sc Merge}$(M_{S'},M_s))$
        % \State $R_S \leftarrow run\_cascade(G,S,J)$
        \State {Compute seed set $S$'s Reachability set $R_{G}(S)$ \kktodo{buna da bir algoritma eklesek} }\ggx{Yayın konusu dışında kalabilir diye çıkartmıştık.}
        ...
        \Else
            \For{$ v\in V$} {\bf in parallel}
                ...
                \EndFor
            \EndFor
            \State $M \leftarrow ${\sc Simulate}$(G,M,R_{S})$
            \State $M_{S'} \leftarrow zeros(J) $ 
...
\end{algorithm}
 ...
 Then, we perform the diffusion process on the sketch registers. 
 To initiate the process, all the vertices are added to the {\em live vertex set} $L$. 
 Then the incoming edges of the live vertices are processed. 
 For all vertices, $u$, its sketch, $M_u$, is updated by merging sketchs $M_v$ of all neighbors vertices $v \in {L}$. Merge result $M[j] = max(M_u[j],M_v[j])$ is written back to $M_u[j]$. This approach can be seen as a bottom-up diffusion process on underlying data-structure. At each iteration, one level of cardinality information is pulled from vertices neighbors to vertices itself.
...
\begin{figure}[!ht]
    ...
    \caption{ Effect of register saturation on Amazon dataset using \acro($J=256$) without rebuilding. \kktodo{buna TIMS'in sonuclarini da ekleyebilir miyiz - en kaliteli o diye, eger mavinin ustunde kaliyorsa guzel} \ggx{Bunun için süreye ihtiyacım var. Diğer algorithmlar mavi çizgi gibi olmalı,(SKIM hariç) }
...
Since building sketches with \acro is very fast, we exploit this by rebuilding sketches after observing large cardinality estimation errors. \acro allows us to work on smaller problem after each rebuilding. We remove influenced vertices from their respective samples and reachability sets are computed on this subgraph. Therefore, we only need to consider {\em latent} seed vertices, $S'$, that are chosen after the sketch rebuild.
