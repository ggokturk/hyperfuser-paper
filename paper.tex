...
The toy example in Fig.~\ref{fig:fmops} shows how the sketch operations are done on a toy problem. In Fig.~\ref{fig:fmops}(a), the sketches, in a toy problem with 4 samples and 4-bit hash functions, are initialized; the id of vertex $u$ is hashed and leading zeros of the hash are written to registers. In Fig.~\ref{fig:fmops}(a), the merge operation is illustrated on registers of vertex $u$, $M_u$, and vertex $v$, $M_v$; merge operation takes element-wise maximum of registers to find union of the reachability sets. Fig.~\ref{fig:fmops} shows writing the merged reachability sets back to registers w.r.t sampling using a conditional move operation. In the example, we see four possible situations that can happen; The operations on the first registers show moving bigger register $M_v[1]$ to $M_u[0]$. The second one shows the opposite, a bigger number is on the target register and there is no edge between $u$ and $v$ in sample graph $\hat{G}$. The third one shows an example when $M_v[3]$ is bigger than $M_u[3]$ but there is no edge between $u$ and $v$ in sample graph $\hat{G}$. The operations on the fourth register show when there is an edge between $u$ and $v$ but $M_u[4]$ is smaller than $M_v[4]$ so the move operation is not needed. 
...
\begin{figure}[!ht]
    ...
    \caption{InFuseR diffusion process illustrated for \emph{only vertex $1$} on toy sample graph.  (a) \label{fig:infuserinit} Toy graph InFuseR initialization.  (b) \label{infuserdiff1} First level of diffusion using \emph{push-based} method for vertex $1$. (c) Second level of diffusion following Fig.\ref{fig:infuserdiff}(a).
...
% Method & \multicolumn{2}{c}{{\sc \acro}} &   {\sc Tim+} &    {\sc Imm} &    {\sc Skim} &     {\sc Ssa} & \multicolumn{2}{c}{{\sc \acro}} &    {\sc Tim+} &     {\sc Imm} &    {\sc Skim} &     {\sc Ssa} & \multicolumn{2}{c}{{\sc \acro}} &  {\sc Tim+} &  {\sc Imm} &   {\sc Skim} &  {\sc Ssa} \\
...
% Method & \multicolumn{2}{c}{{\sc \acro}} &   {\sc Tim+} &     {\sc Imm} &     {\sc Ssa} & \multicolumn{2}{c}{{\sc \acro}} &    {\sc Tim+} &     {\sc Imm} &     {\sc Ssa} & \multicolumn{2}{c}{{\sc \acro}} &  {\sc Tim+} &   {\sc Imm} &   {\sc Ssa} \\
...
% \toprule
% {} & \multicolumn{5}{c|}{Time} & \multicolumn{5}{c|}{Score} & \multicolumn{5}{c}{Memory} \\
% Method & {\sc \acro} &   {\sc Tim+} &    {\sc Imm} &    {\sc Skim} &     {\sc Ssa} & {\sc \acro} &    {\sc Tim+} &     {\sc Imm} &    {\sc Skim} &     {\sc Ssa} & {\sc \acro} &  {\sc Tim+} &  {\sc Imm} &   {\sc Skim} &  {\sc Ssa} \\
% Dataset                 &            &        &        &         &         &            &         &         &         &         &            &       &      &        &      \\
% \midrule
...
\begin{table*}
    ...
    \setlength{\tabcolsep}{1.2pt}
        ...
        \label{tab:timings0.005}
        \centering
        \resizebox{\resultsize\linewidth}{!}{
    
\begin{tabular}{l|rrrrr|rrrrr|rrrrr}
\toprule
{} & \multicolumn{5}{c|}{Time} & \multicolumn{5}{c|}{Score} & \multicolumn{5}{c}{Memory} \\
Method & {\sc Hyp.} &   {\sc Tim+} &    {\sc Imm} &    {\sc Skim} &     {\sc Ssa} &  {\sc Hyp.}   &    {\sc Tim+} &     {\sc Imm} &    {\sc Skim} &     {\sc Ssa} & {\sc Hyp.}  &  {\sc Tim+} &  {\sc Imm} &   {\sc Skim} &  {\sc Ssa} \\
...
\toprule
{} & \multicolumn{5}{c|}{Time} & \multicolumn{5}{c|}{Score} & \multicolumn{5}{c}{Memory} \\
Method & \acrotwo &    {\sc Tim+} &    {\sc Imm} &    {\sc Skim} &     {\sc Ssa} &  \acrotwo &    {\sc Tim+} &     {\sc Imm} &    {\sc Skim} &     {\sc Ssa} & \acrotwo &  {\sc Tim+} &   {\sc Imm} &  {\sc Skim} &   {\sc Ssa} \\
Dataset                 &            &         &        &         &         &             &         &         &         &         &            &       &       &       &       \\
\midrule
